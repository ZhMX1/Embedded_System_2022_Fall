...
#if 0
	#define MP3_DISK	FS_VOLUME_SD		/* 选择缺省读取的磁盘:  0是SD卡 */
	#define MP3_PATH	"0:/Music"			/* MP3文件存放的缺省文件夹， 根目录下的Music */
#else
	#define MP3_DISK	FS_VOLUME_NAND		/* 选择缺省读取的磁盘:  1是NAND */
	#define MP3_PATH	"1:/Music"			/* MP3文件存放的缺省文件夹， 根目录下的Music */
...
void MP3Player(void)
{
	...
	/* 打印NAND Flash 根目录和MP3目录下的文件 */
	{
		...
		sprintf(path, "%s", MP3_DISK);		/* 根目录 */
		ViewDir(path);

		sprintf(path, "%s", MP3_PATH);	/* 音乐目录 */
		ViewDir(path);
	}
		...
		if (ucKeyCode > 0)
		{
			...
			switch (ucKeyCode)
			{
				...
				case KEY_DOWN_K2:			/* K2键按下 */
					f_lseek(&FormMP3->file, 0);	/* 修改文件当前指针到文件头, 从头开始播放 */
					g_tMP3.uiProgress = 0;	/* 进度 */
					ucRefresh = 1;
					break;
