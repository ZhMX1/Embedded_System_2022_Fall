...
void RadioMain(void)
{
...

	{
		...
		/* 预填武汉地区的电台列表 */
		{
			if (g_pFM->ucListType == 0)	/* 0 表示 武汉地区列表 1 表示全国列表 */
			{
			...
			//g_pFM->ucMode = FM_RX;	/* 缺省是FM接收 */
			//g_pFM->ucMode = AM_RX;	/* 缺省是FM接收 */
		}

		bsp_DelayMS(300);	/* 必须延迟一段时间后，才能保证每次上电后，能够选中一个台 */
		if (g_pFM->ucMode == FM_RX)
		{
			g_pFM->usFreq = g_pFM->usFMList[g_pFM->ucIndexFM];
			SI4730_SetFMFreq(g_pFM->usFreq);
		}
		else
		{
			g_pFM->usFreq = g_pFM->usAMList[g_pFM->ucIndexAM];
			SI4730_SetAMFreq(g_pFM->usFreq);
		}
				...
				case TOUCH_RELEASE:		/* 触笔释放事件 */
					...
					else if (TOUCH_InRect(tpX, tpY, BUTTON_FM_X, BUTTON_FM_Y, BUTTON_FM_H, BUTTON_FM_W))
					{
						...
						{						
							...
							SI4730_SetOutVolume(g_pFM->ucVolume);	/* 缺省音量是最大值63, 修改为32 */
			...
			*/
			switch (ucKeyCode)
			{
				...
				case JOY_DOWN_U:			/* 摇杆UP键按下 */
					g_pFM->ucVolume += VOLUME_STEP;
					if (g_pFM->ucVolume > VOLUME_MAX)
					{
						g_pFM->ucVolume = VOLUME_MAX;
					}
					radio_AdjustVolume(g_pFM->ucVolume);				
					fRefresh = 1;
					break;
				...
				case JOY_DOWN_R:			/* 摇杆RIGHT键按下 */
					if (g_pFM->ucMode == FM_RX)
					{
						if (g_pFM->ucIndexFM < g_pFM->ucFMCount - 1)
						{
							g_pFM->ucIndexFM++;
							g_pFM->usFreq = g_pFM->usFMList[g_pFM->ucIndexFM];
							SI4730_SetFMFreq(g_pFM->usFreq );
					...
					{
						if (g_pFM->ucIndexAM < g_pFM->ucAMCount - 1)
						{
							g_pFM->ucIndexAM++;
							g_pFM->usFreq = g_pFM->usAMList[g_pFM->ucIndexAM];
							SI4730_SetAMFreq(g_pFM->usFreq );
...
static void radio_DispStatus(void)
{
	...
	if (g_pFM->ucMode == FM_RX)
	{
		sprintf(buf, "FM (%3d/%d) 频率=%5d.%dMHz, Volume = %2d    ", g_pFM->ucIndexFM + 1,
			g_pFM->ucFMCount, g_pFM->usFreq / 100,
			(g_pFM->usFreq % 100) / 10, g_pFM->ucVolume);
	}
	else
	{
		sprintf(buf, "AM (%3d/%d) 频率=%5dKHz, Volume = %2d    ",g_pFM->ucIndexAM + 1,
			g_pFM->ucAMCount,	g_pFM->usFreq, g_pFM->ucVolume);
	}
...
static void radio_FM_FreqList(uint8_t _ucAll)
{
	...
	if (_ucAll == 0)
	{
		g_pFM->ucFMCount = sizeof(g_InitListFM) / 2;
		for (i = 0 ;i < g_pFM->ucFMCount; i ++)
		{
			g_pFM->usFMList[i] = g_InitListFM[i];
		}
	}
	else
	{
		...
		*/
		...
		{
			g_pFM->usFMList[g_pFM->ucFMCount++] = i;
		}
	}
}
...
static void radio_AM_FreqList(uint8_t _ucAll)
{
	...
	if (_ucAll == 0)
	{
		g_pFM->ucAMCount = sizeof(g_InitListAM) / 2;
		for (i = 0 ;i < g_pFM->ucAMCount; i ++)
		{
			g_pFM->usAMList[i] = g_InitListAM[i];
		}
	}
	else
	{
...
static void radio_LoadParam(void)
{
	...
	g_pFM->ucMode       = g_tParam.ucRadioMode;			/* AM 或 FM */
	g_pFM->ucListType   = g_tParam.ucRadioListType;		/* 电台列表类型。武汉地区或全国 */
	g_pFM->ucIndexFM    = g_tParam.ucIndexFM;				/* 当前FM电台索引 */
	g_pFM->ucIndexAM    = g_tParam.ucIndexAM;				/* 当前电台索引 */
	g_pFM->ucVolume     = g_tParam.ucRadioVolume;			/* 音量 */
	g_pFM->ucSpkOutEn   = g_tParam.ucSpkOutEn;			/* 扬声器输出使能 */
}
...
static void radio_SaveParam(void)
{
	g_tParam.ucRadioMode     = g_pFM->ucMode;				/* AM 或 FM */
	g_tParam.ucRadioListType = g_pFM->ucListType;			/* 电台列表类型。武汉地区或全国 */
	g_tParam.ucIndexFM       = g_pFM->ucIndexFM;			/* 当前FM电台索引 */
	g_tParam.ucIndexAM       = g_pFM->ucIndexAM;			/* 当前电台索引 */
	g_tParam.ucRadioVolume   = g_pFM->ucVolume;			/* 音量 */
	g_tParam.ucSpkOutEn      = g_pFM->ucSpkOutEn;			/* 扬声器输出使能 */
...
static void radio_AdjustVolume(uint8_t _ucVolume)
{
	...
	wm8978_SetEarVolume(g_pFM->ucVolume);
	wm8978_SetSpkVolume(g_pFM->ucVolume);
