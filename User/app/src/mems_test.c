...
#define BOX3_H	52
#define BOX3_W	220
#define BOX3_TEXT	"BMP180 气压计和温度"
...
#define BOX6_X	(BOX1_X)
#define BOX6_Y	(BOX3_Y + BOX3_H + 5)
#define BOX6_H	52
#define BOX6_W	220+220
...
//	}
	...
	bsp_InitBMP180();
	BMP180_ReadTempPress();		/* 读取结果存放在全局变量 */
...
static void DispBMP180(void)
{
	...
	/* 温度值， 单位 0.1摄氏度 */
	/* 压力值， 单位 Pa */
	sprintf(buf, "环境温度  = %3d.%d℃", g_tBMP180.Temp / 10, g_tBMP180.Temp % 10);
	LCD_DispStr(x, y, buf, &tFont);
	y += ucLineCap;

	sprintf(buf, "大气压力  = %3d.%03dKPa", g_tBMP180.Press / 1000, g_tBMP180.Press % 1000);
	LCD_DispStr(x, y, buf, &tFont);
	y += ucLineCap;
}
