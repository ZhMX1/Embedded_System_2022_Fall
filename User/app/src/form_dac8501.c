...
typedef struct
{
	...
	uint16_t s_WaveBuf[WAVE_SAMPLES];
	uint16_t s_WavePos1, s_WavePos2;
}Form8501_T;
...
void FormMainDAC8501(void)
{
	...
	MakeSinTable(Form8501->s_WaveBuf, WAVE_SAMPLES, 0, 65535);
			...
			switch (ucTouch)
			{
				case TOUCH_DOWN:		/* 触笔按下事件 */
					...
					else if (LCD_ButtonTouchDown(&Form8501->Btn12, tpX, tpY))
					{
						...
						Form8501->s_WavePos1 = 0;					/* 波形1超前 0度 */
						Form8501->s_WavePos2 = 0;
					}
					else if (LCD_ButtonTouchDown(&Form8501->Btn13, tpX, tpY))
					{
						bsp_SetTIMforInt(TIM4, DAC_OUT_FREQ, 0, 0);
						Form8501->s_WavePos1 = WAVE_SAMPLES / 4;	/* 波形1超前 90度 */
						Form8501->s_WavePos2 = 0;
					}
					else if (LCD_ButtonTouchDown(&Form8501->Btn14, tpX, tpY))
					{
						bsp_SetTIMforInt(TIM4, DAC_OUT_FREQ, 0, 0);
						Form8501->s_WavePos1 = WAVE_SAMPLES / 2;	/* 波形1超前 180度 */
						Form8501->s_WavePos2 = 0;
					}
...
void TIM4_IRQHandler(void)
{
	...
	if (TIM_GetITStatus(TIM4, TIM_IT_Update) != RESET)
	{
		...
		/* 波形1 */
		dac = Form8501->s_WaveBuf[Form8501->s_WavePos1++];
		if (Form8501->s_WavePos1 >= WAVE_SAMPLES)
		{
			Form8501->s_WavePos1 = 0;
		}
		DAC8501_SetDacData(0, dac);		/* 改变第1通道 DAC输出电压 */

		/* 波形1 */
		dac = Form8501->s_WaveBuf[Form8501->s_WavePos2++];
		if (Form8501->s_WavePos2 >= WAVE_SAMPLES)
		{
			Form8501->s_WavePos2 = 0;
		}
