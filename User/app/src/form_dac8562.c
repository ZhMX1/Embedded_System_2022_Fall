...
void FormMainDAC8562(void)
{
	...
	MakeSinTable(Form8562->s_WaveBuf, WAVE_SAMPLES, 0, 65535);
			...
			switch (ucTouch)
			{
				case TOUCH_DOWN:		/* 触笔按下事件 */
					...
					else if (LCD_ButtonTouchDown(&Form8562->Btn12, tpX, tpY))
					{
						...
						Form8562->s_WavePos1 = 0;					/* 波形1超前 0度 */
						Form8562->s_WavePos2 = 0;
					}
					else if (LCD_ButtonTouchDown(&Form8562->Btn13, tpX, tpY))
					{
						bsp_SetTIMforInt(TIM7, DAC_OUT_FREQ, 0, 0);
						Form8562->s_WavePos1 = WAVE_SAMPLES / 4;	/* 波形1超前 90度 */
						Form8562->s_WavePos2 = 0;
					}
					else if (LCD_ButtonTouchDown(&Form8562->Btn14, tpX, tpY))
					{
						bsp_SetTIMforInt(TIM7, DAC_OUT_FREQ, 0, 0);
						Form8562->s_WavePos1 = WAVE_SAMPLES / 2;	/* 波形1超前 180度 */
						Form8562->s_WavePos2 = 0;
					}
...
void TIM7_IRQHandler(void)
{
	...
	if (TIM_GetITStatus(TIM7, TIM_IT_Update) != RESET)
	{
		...
		/* 波形1 */
		dac = Form8562->s_WaveBuf[Form8562->s_WavePos1++];
		if (Form8562->s_WavePos1 >= WAVE_SAMPLES)
		{
			Form8562->s_WavePos1 = 0;
		}
		DAC8562_SetDacData(0, dac);		/* 改变第1通道 DAC输出电压 */

		/* 波形1 */
		dac = Form8562->s_WaveBuf[Form8562->s_WavePos2++];
		if (Form8562->s_WavePos2 >= WAVE_SAMPLES)
		{
			Form8562->s_WavePos2 = 0;
		}
