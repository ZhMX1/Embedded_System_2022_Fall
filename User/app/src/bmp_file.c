...
void SaveScreenToBmp(uint16_t _index)
{
	...
	h = LCD_GetHeight();
	w = LCD_GetWidth();
	MakeBmpHeader(&tBmpHead, w, h);	/* 构建BMP文件头 */
	...
	/* 文件系统操作 */
	{
		...
		/* 打开文件 */
		sprintf(path, "%s/%02d_%dx%d.bmp", FS_VOLUME_SD, _index, tBmpHead.biWidth, tBmpHead.biHeight);
	...
	for (i = 0; i <  tBmpHead.biHeight; i++)
	{
		...
		for (j = 0; j < tBmpHead.biWidth; j++)
		{
		...
		#else
			/* 分批次写入， 每次不能超过512字节 */
			bytes = tBmpHead.biWidth * 3;
			for (j = 0 ;j < bytes / 512; j++)
			{
				/* 每次写入512字节 */
				result = f_write(&file, &LineData[512 * j], 512, &bw);
				if (result != FR_OK)
				{
					printf("%s 文件写入失败\r\n", path);
					goto quit1;
			...
			if (bytes % 512)
			{
				result = f_write(&file, &LineData[(bytes / 512) * 512], bytes % 512, &bw);
				if (result != FR_OK)
				{
					printf("%s 文件写入失败\r\n", path);
					goto quit1;
